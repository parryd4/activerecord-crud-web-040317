...
def can_find_the_first_item_from_the_database_using_id
  Movie.find(Movie.ids.first).title
end
...
def can_find_by_multiple_attributes
  ...
  # release_date == 2000
  # director == "Me"
  hash_conditions = {title: "Title",
    release_date: 2000, director: "Me"
    }
  Movie.where(hash_conditions).first
  # without [0] returns ActiveRecord::Relation
...
def can_be_found_updated_and_saved
  ...
  temp = Movie.find_by(title: "Awesome Flick")
  temp.title = "Even Awesomer Flick"
  temp.save
end
...
def can_update_using_update_method
  ...
  grab_id = Movie.find_by(title: "Wat?").id
  Movie.update(grab_id, title: "Wat, huh?")
end
